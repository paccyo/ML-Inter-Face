...
def evaluate(model, mode='validation', alg=None, data=None, target=None, export_path=None):
        if mode == 'validation':
        ...
        if alg == 'randomforest':
            ...
            dot_data = export_graphviz(estimators[0], # 決定木オブジェクト
                                        ...
                                        feature_names=data.columns, #説明変数
        ...
        if alg == 'decisiontree':
            plot_tree(model, feature_names=data.columns, class_names=model.classes_, filled=True)
            plt.show()