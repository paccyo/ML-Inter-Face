...
class ModelBuild(ft.Tab):
    ...
    def update_connect_layer(self, layers):
    ...
    def on_drag_update(self, e: ft.DragUpdateEvent):
        ...
        layer = sorted(self.design_area.content.content.controls[1:], key = lambda x:x.top)
        self.update_connect_layer(layer)

    def on_tap_layer(self, e):
        data = dicts
        def on_change_params(e_control, control_data):
        ...
        for param, value in layer_params.items():
            ...
            if control_type == TEXTFIELD:
                rect = ft.Row([
                    ...
                    ft.TextField(value=param_value,
                                border="underline",
                                text_size=20,
